...
async def chart_default(ctx, symbol: str = 'AAPL'):
    ...
    buf = make_chart(df, symbol, f'1wk')
    if buf is None:
                      await ctx.send(f"No data available for {symbol}. Check if the symbol is correct.")
                      return
...
async def chart_minute(ctx, minutes: int = 5, symbol: str = 'AAPL'):
    ...
    if buf is None:
                      await ctx.send(f"No intraday data available for {symbol}. Market may be closed or symbol invalid.")
                      return
                  await ctx.send(file=discord.File(buf, f'{symbol}_chart.png'))
